...
int main_test(int argc, char** argv) {
    ...
    openURL(*env, "testRTSPClient", "rtsp://192.168.1.134/test.aac");
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
    ...
    auto to_hex = [](const uint8_t* buff, int len) -> std::string {
    ...
    std::cout << to_hex(fReceiveBuffer,frameSize) << std::endl;
    
    
    if (strcmp(fSubsession.mediumName(), "video") == 0
        && strcmp(fSubsession.codecName(), "H264") == 0) {
        envir() << "This is video h264 packet" << "\n";
        if (first_frame)
        {
            first_frame = false;
            ...
            decoder.open_codec("H264","", sps, spsSize, pps, ppsSize);
        }
       
        decoder.decode_video_rtp(fReceiveBuffer, frameSize);
    }else if (strcmp(fSubsession.mediumName(), "audio") == 0 ) {
        envir() << "This is audio AAC packet" << "\n";
        
        if (first_frame) {
            decoder.open_codec("",fSubsession.codecName());
            first_frame = false;
        }
        decoder.decode_audio_rtp(fReceiveBuffer, frameSize);
    }else {
        envir() << "media name:" << fSubsession.mediumName() << ", codec:" << fSubsession.codecName() << "\n";
    }
